...
var kid1Hash = '80ea8bc8a58f990ad1f76bc665b30bfa'

var wrappedKey2 = 'ffaf1dae9201d1adf62770dca5ddb77ad773a79369e39986' // key2 wrapped with kek3
...
var test1 = makeTest(
  ...
  function (callback) {
    async.eachSeries([kid1, kid2, kid3, kid4, kid1Hash], function (item, callback) {
...
var test12 = makeTest(
  ...
  function (callback) {
    callApi('/keys?kek=' + kek1, 'POST', '{"kid":"^kid1"}', callback, function (response) {
      ...
      var key = JSON.parse(response.body)
      checkEqualNoCase('KID value', kid1Hash, key.kid)
      keyTable[key.kid] = key.k
    })
  })
...
var test16 = makeTest(
  ...
  function (callback) {
    callApi('/keys/' + kid4, 'PUT', '{"ek":"' + wrappedKey2 + '"}', callback, function (response) {
...
var test17 = makeTest(
  ...
  function (callback) {
    callApi('/keys', 'POST', '{"kid":"' + kid4 + '", "ek":"' + wrappedKey2 + '"}', callback, function (response) {
      checkEqual('HTTP status code', response.statusCode, 201)
      checkResponseMimeType(response)
      keyTable[kid4] = key2
    })
...
var test18 = makeTest(
  ...
  function (callback) {
    callApi('/keys/' + kid4, 'GET', null, callback, function (response) {
      ...
      checkResponseMimeType(response)
      var keys = JSON.parse(response.body)
      checkEqualNoCase('Encrypted key', keys.ek, wrappedKey2)
    })
  })
