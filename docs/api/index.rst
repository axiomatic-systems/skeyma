...
.. http:post:: /keys/{kid}
  ...
  A partial Key object is one where not all fields are set. Fields that are not set are automatically generated by the server:
  ...
  **Example request: create a new object with all fields already set**:
...
.. http:get:: /keys/{kid}/value
  ...
  If a KEK is passed as a query parameter, the response body contains the raw cleartext value of the key. If no KEK is passed, the response body contains the wrapped key, prefixed with the characater '#'
